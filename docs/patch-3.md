...
| 通道 | 地址 | 覆盖 |
| --- | --- | --- |
| 同源 HTTP（首选） | `POST https://poem.aibeaver.cn/api/ingest`、`GET\|POST https://poem.aibeaver.cn/api/usage` | 提交课包、拿回使用数据 |
| MCP（可选） | `https://poem.aibeaver.cn/mcp` | 同上，另含名册：`list_roster` / `upsert_poet` / `upsert_work` |
...
**HTTP 覆盖不到名册**：新诗人 / 头像（`upsert_poet`）与查名册（`list_roster`）只有 MCP 有。HTTP 提交若被拒成「诗人不在名册中」，按第 3 节接上 MCP，或请老师用管理台加。
